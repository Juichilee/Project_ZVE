...
public class RootMotionControlScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        //work
        // isGrounded = IsGrounded || CharacterCommon.CheckGroundNear(this.transform.position, jumpableGroundNormalMaxAngle, 0.1f, 1f, out closeToJumpableGround);
        ...
        // get info about current aimation
        ...
        // The conditions tell us what animation is playing, if we're in a transition, and if we aren't already matching targets
        if (animState.IsName("MatchToButtonPress") && !anim.IsInTransition(0) && !anim.isMatchingTarget){
            if (buttonPressStandingSpot != null) {
                ...
                // MatchTarget() allows us to reference a current animation to serve as a time line.
                // Over the progression of this timeline, we will override root motion and interpolate towards
                // a desired reference point and which dimensions are used via MatchTargetWeightMask. 
                // The time period of the interpolation is controlled by a normalized time reference for the start and stop
        ...
        curVelocity = (transform.position - prevVelocity) / Time.fixedDeltaTime;
        prevVelocity = transform.position;
        float normalizedVerticalSpeed = (curVelocity.y + maxVerticalSpeed) / (2 * maxVerticalSpeed);

        anim.SetFloat("velx", _inputTurn);
        anim.SetFloat("vely", _inputForward);
        anim.SetFloat("velz", normalizedVerticalSpeed);
